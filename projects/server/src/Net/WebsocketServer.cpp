...
namespace JourneyOfDreams
{
  ...
  WebsocketServer::WebsocketServer() {
    ...
    this->endpoint.set_reuse_addr(true); // Allow the server to be restarted
  ...
  WebsocketServer::onClose(ClientConnection conn) {
    {
      ...
      auto newEnd = std::remove_if(this->openConnections.begin(),
                                   ...
                                   [&connVal](ClientConnection elem) {
                                     // If the pointer has expired, remove it
                                     // from the vector
                                     if (elem.expired() == true) {
                                       return true;
                                     }

                                     // If the pointer is still valid, compare
                                     // it to the handle for the closed
                                     // connection
