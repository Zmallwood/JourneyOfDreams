...
namespace JourneyOfDreams
{
  RegisterScene::RegisterScene() {
    ...
    registerPanel->AddWidget(std::make_shared<GUIButton>(
        RectF{0.22f, 0.3f, 0.1f, 0.05f}, "Register", [this] {
          if (EnsurePasswordConfirmed()) {
            ...
            GUI()->AddWidget("MsgBoxIncorrectPasswordConfirmation",
                             std::make_shared<GUIMessageBox>(
                                 PointF{.x = 0.5f, .y = 0.5f},
                                 "Your typed passwords do not match.",
                                 "Error"));
          }
        }));
    registerPanel->AddWidget(std::make_shared<GUIButton>(
        RectF{0.014f, 0.3f, 0.1f, 0.05f}, "Back",
        [] { _<SceneManager>().GoToScene("LoginScene"); }));
    registerPanel->AddWidget(std::make_shared<GUILabel>(
        PointF{0.02f, 0.05f}, "Username", Colors::Silver));
    registerPanel->AddWidget(
        "UsernameTextBox",
        std::make_shared<GUITextBox>(RectF{0.15f, 0.05f, 0.18f, 0.05f}));
    registerPanel->AddWidget(std::make_shared<GUILabel>(
        PointF{0.02f, 0.12f}, "Password", Colors::Silver));
    registerPanel->AddWidget(
        "PasswordTextBox",
        std::make_shared<GUITextBox>(RectF{0.15f, 0.12f, 0.18f, 0.05f},
                                     Colors::Wheat, true));
    registerPanel->AddWidget(std::make_shared<GUILabel>(
        PointF{0.02f, 0.19f}, "Confirm", Colors::Silver));
    registerPanel->AddWidget(
        "ConfirmPasswordTextBox",
        std::make_shared<GUITextBox>(RectF{0.15f, 0.19f, 0.18f, 0.05f},
                                     Colors::Wheat, true));
  }
