...
namespace zw
{
    WorldView::WorldView(Engine &engine)
        : m_ridsTiles(std::make_shared<std::vector<std::vector<RID>>>()),
          m_ridsObjects(std::make_shared<std::vector<std::vector<RID>>>()), m_engine(engine)
    {
    ...
    void WorldView::Render()
    {
        ...
        //         auto hoveredCoord = m_engine.SceneManager()->GetScene<MainScene>("MainScene").TileHoverer()->HoveredCoordinate();
