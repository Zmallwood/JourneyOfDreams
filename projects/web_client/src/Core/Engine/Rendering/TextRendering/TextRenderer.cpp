...
namespace JourneyOfDreams
{
  ...
  void TextRenderer::RenderText(RID rid, const std::string &text, ColorF color,
    ...
    auto textOutlineSurf =
        TTF_RenderText_Blended(m_fonts.at(fontSize)->OutlineSDLFont().get(),
    ...
    auto img = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 32, 0x000000FF,
    ...
    textSrcRect =
        SDL_Rect{0, 0, textSurf ? textSurf->w : 0, textSurf ? textSurf->h : 0};
    textOutlineSrcRect =
        SDL_Rect{0, 0, textOutlineSurf ? textOutlineSurf->w : 0,
    ...
    SDL_BlitSurface(textOutlineSurf, &textOutlineSrcRect, img,
                    &textOutlineDestRect);
  ...
  void TextRenderer::DrawString(RID rid, const std::string &text,
    ...
    int text_w;
    int text_h;
    TTF_SizeText(m_fonts.at(fontSize)->SDLFont().get(), text.data(), &text_w,
                 &text_h);
    // rect.h *= GetAspectRatio(_<GraphicsView>()->Window());
    // rect.y -= static_cast<float>(text_h) / canvSz.h / 2.0f;

    if (centerAlign) {
      rect.x -= static_cast<float>(text_w) / static_cast<float>(canvSz.h) /
                2.0f / GetAspectRatio();
      rect.y -=
          static_cast<float>(text_h) / static_cast<float>(canvSz.h) / 2.0f;
    }
