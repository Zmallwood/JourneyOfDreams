...
namespace zw {
  GUIButton::GUIButton(zw::Engine& engine, RectF area, const std::string &text, std::function<void()> onClick, ColorF textColor)
      : GUIWidget(engine, area), m_text(text), m_textColor(textColor), m_onClick(std::move(onClick)) {
  ...
  void GUIButton::UpdateDerived() {
    // auto mousePos = GetMousePosition(Engine());
