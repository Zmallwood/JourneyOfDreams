...
namespace zw {
  GUILabel::GUILabel(zw::Engine& engine, PointF position, const std::string &text, GUIAlign alignment, FontSizes fontSize,
                     ColorF textColor)
      : GUIWidget(engine, position, engine.TextRenderer()->MeasureString(text, fontSize), alignment), m_text(std::make_shared<std::string>(text)),
  ...
  GUILabel::GUILabel(zw::Engine& engine, PointF position, const std::string &text, ColorF textColor)
      : GUILabel(engine, position, text, GUIAlign::TopLeft, FontSizes::_20, textColor) {
  }
